...
function resolveLuckyOpenTokenFromEnv(prefix = 'LUCKY') {
  const candidates = [
    process.env[`${prefix}_OPEN_TOKEN`],
  ...
  if (config?.modules?.lucky) {
    ...
    if (process.env.LUCKY_API_BASE) {
      luckyInstances.push({
        ...
        openToken: resolveLuckyOpenTokenFromEnv('LUCKY'),
        token: process.env.LUCKY_TOKEN || process.env.LUCKY_API_TOKEN,
        adminToken: process.env.LUCKY_ADMIN_TOKEN,
    ...
    if (process.env.LUCKY_BACKUP_API_BASE) {
      luckyInstances.push({
        ...
        openToken: resolveLuckyOpenTokenFromEnv('LUCKY_BACKUP'),

...
async function loadConfig() {
  try {
    ...
    return {
      lucky: {
        apiBase: process.env.LUCKY_API_BASE || config.lucky?.apiBase || 'http://192.168.9.2:16601/666',
        openToken: process.env.LUCKY_OPEN_TOKEN || config.lucky?.openToken
      },
      sunpanel: {
        apiBase: process.env.SUNPANEL_API_BASE || config.sunpanel?.apiBase || 'http://192.168.9.2:20001/openapi/v1',

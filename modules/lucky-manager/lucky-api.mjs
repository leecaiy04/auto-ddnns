...
async function luckyFetch(url, options = {}, config = null) {
  ...
  if (requestBody && !("Content-Length" in requestOptions.headers) && !("content-length" in requestOptions.headers)) {
  ...
  const envRequestTimeoutMs = Number.parseInt(getEnv('LUCKY_REQUEST_TIMEOUT_MS', '20000'), 10);
  const requestTimeoutMs = options.requestTimeoutMs ?? (Number.isFinite(envRequestTimeoutMs) && envRequestTimeoutMs > 0 ? envRequestTimeoutMs : 20000);
    ...
    try {
      return await new Promise((resolve, reject) => {
        ...
        req.setTimeout(requestTimeoutMs, () => {
          req.destroy(new Error(`Lucky API 请求超时 (${requestTimeoutMs}ms)`));
        });

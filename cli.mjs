  ...
  try {
    switch (action) {
      ...
      case 'sync-cloudflare':
        ...
        await modules.serviceRegistry.loadRegistry();
        await modules.cloudflareManager.syncServicesToCF(modules.serviceRegistry.getAllServices());
        break;
      case 'sync-sunpanel':
...
支持的任务:
  sync-all          执行完整同步 (设备发现 -> DDNS -> Lucky -> SunPanel -> Cloudflare)
  ...
  import-lucky      从 Lucky 现有反代规则导入服务清单

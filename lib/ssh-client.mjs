...
export function executeSSHCommand(command, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      finish(() => reject(new Error(`SSH 命令执行超时 (${timeout}ms)`)));
    }, timeout);
        ...
        setTimeout(() => {
          ...
          const cleaned = lines.filter(line => {
            ...
            return trimmed &&
              ...
              !trimmed.startsWith('WAP>') &&
              !trimmed.startsWith('Last login:') &&
              trimmed !== 'success!' &&
              trimmed !== 'Configuration console exit, please retry to log on' &&
              !trimmed.endsWith('#') &&
              !trimmed.endsWith('$');
          });
          ...
          if (!stdout && stderr) {
            finish(() => reject(new Error(stderr)));
            return;
          }
    ...
    conn.on('error', (err) => {
      finish(() => reject(new Error(`SSH 连接失败: ${err.message}`)));
    });
...
export async function getIPv6Neighbors(options = {}) {
  try {
    ...
    for (const line of lines) {
      ...
      if (!/\sdev\s+\S+\s+lladdr\s+[0-9a-f:]{17}\s+/i.test(line)) {
        continue;
      }
      ...
      const state = parts.at(-1); // 状态通常在末尾: REACHABLE/STALE/FAILED

      if (!ipv6.includes(':')) continue;
      if (!/^[0-9a-f:]+$/i.test(ipv6)) continue;
...
export async function getARPTable(options = {}) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 解析格式: 192.168.3.10 dev br0 lladdr xx:xx:xx:xx:xx:xx ... REACHABLE
      const parts = line.split(/\s+/);
      if (parts.length < 6) continue;

      const ip = parts[0];
      const device = parts[2]; // dev 后面是设备名
      const mac = parts[4]; // lladdr 后面是 MAC 地址

      if (!/^\d{1,3}(\.\d{1,3}){3}$/.test(ip)) continue;
      if (!ip.startsWith('192.168.3.')) continue;
      if (!/^[0-9a-f]{2}(:[0-9a-f]{2}){5}$/i.test(mac)) continue;
...
export async function buildDeviceAddressMap(options = {}) {
  try {
    // 路由器 CLI 对并发 SSH session 不稳定，顺序抓取更可靠
    const arpEntries = await getARPTable(options);
    const neighbors = await getIPv6Neighbors(options);

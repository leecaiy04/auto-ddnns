...
const DEFAULT_ADMIN_API_BASE_URL = 'https://lucky.leecaiy.shop:50000/666';
const DEFAULT_OPEN_API_BASE_URL = 'http://192.168.3.200:16601';

export const getOpenToken = () => getEnv('LUCKY_OPEN_TOKEN', '');
export const getAdminToken = () => getEnv('LUCKY_ADMIN_TOKEN', getOpenToken());
...
export const getApiBaseUrl = () => {
  const configuredBase = getEnv('LUCKY_API_BASE', '');
  if (configuredBase) {
    return configuredBase;
  }

  return getLuckyAuthMode() === 'open'
...
export const withOpenToken = (url) => url;
...
function buildAuthHeaders(options = {}) {
  ...
  if (authMode === 'open') {
    return {
      ...
      'Authorization': `Bearer ${getOpenToken()}`,
      ...options.headers
    };
  }

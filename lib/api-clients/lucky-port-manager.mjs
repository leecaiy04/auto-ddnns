...
async function getRawRuleByPort(port) {
  const data = await getReverseProxyRules();
...
function buildUpdatedSubRule(existingSubRule, {
  ...
  return {
    ...
    Domains: domains,
    Locations: locations,
    LocationInsecureSkipVerify: existingSubRule?.LocationInsecureSkipVerify ?? true,
    EnableAccessLog: existingSubRule?.EnableAccessLog ?? true,
    CorazaWAFInstance: existingSubRule?.CorazaWAFInstance ?? '',
    ...
    SafeIPMode: existingSubRule?.SafeIPMode ?? 'blacklist',
    SafeUserAgentMode: existingSubRule?.SafeUserAgentMode ?? 'blacklist',
    UserAgentfilter: existingSubRule?.UserAgentfilter ?? [],
    EasyLucky: existingSubRule?.EasyLucky ?? true,
...
export async function smartAddOrUpdateSubRule(port, remark, serviceType, domains, locations, options = {}) {
  ...
  // 2. 获取端口原始规则
  const rawRule = await getRawRuleByPort(port);
  if (!rawRule) {
    return {
      ret: -1,

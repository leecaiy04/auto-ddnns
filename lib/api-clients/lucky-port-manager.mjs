...
export async function listAllPorts(config = null) {
  ...
  const ruleList = ensureRuleListResponse(data);
...
export async function smartAddOrUpdateSubRule(port, remark, serviceType, domains, locations, options = {}, config = null) {
  ...
  if (!rawRule) {
    const createResult = await createPort(port, `${port}端口`, {
      enableTLS: options.tls !== undefined ? options.tls !== false : false
    }, config);

    if (createResult.ret !== 0) {
      return {
        ret: createResult.ret,
        msg: `创建端口 ${port} 失败: ${createResult.msg || '未知错误'}`,
        action: 'create_failed'
      };
    }
    ...
    if (!rawRule) {
      return {
        ret: -1,
        msg: `端口 ${port} 创建成功但未找到原始规则`,
        action: 'create_missing_rule'
      };
    }
  }

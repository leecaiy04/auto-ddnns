 ...
 */
export async function listAllPorts() {
 ...
 */
export async function getPortDetail(port) {
 ...
 */
export async function smartCreateOrAddProxy(port, name, domain, target, options = {}) {
  // 0. ⚠️ 白名单安全检查
  try {
  ...
  // 1. 检查端口是否已存在
  const existingPort = await getPortDetail(port);
  ...
  // 2. 端口不存在，创建新端口并添加代理
  const createResult = await createPort(port, name, options);
  ...
  // 3. 添加反向代理子规则
  const addResult = await addReverseProxy(ruleKey, name, domain, target);
 ...
 */
export async function createPortWithProxy(port, name, domain, target, options = {}) {
  // 1. 创建端口
  const createResult = await createPort(port, name, options);
  if (createResult.ret !== 0) {
    return createResult;
 ...
 */
export async function findPortByName(name) {
  const ports = await listAllPorts();
 ...
 */
export async function findPortsByDomain(domain) {
  const ports = await listAllPorts();
 ...
 */
export async function searchProxies(keyword) {
...
export async function getPortStats() {
  ...
  return {
    ...
    disabledPorts: ports.filter(p => p => !p.enabled).length,
 ...
 */
export async function exportPortsConfig(port) {
  if (port) {
    const detail = await getPortDetail(port);
 ...
 */
export async function smartAddOrUpdateSubRule(port, remark, serviceType, domains, locations, options = {}) {
  // 1. 白名单检查
  try {
    checkPortWhitelist(port);
  } catch (error) {
  ...
  // 2. 获取端口信息
  const portInfo = await getPortDetail(port);
  if (!portInfo) {
    return {
      ret: -1,
      msg: `端口 ${port} 不存在`,
  ...
  // 3. 检查是否存在同名子规则
  const existingSubRule = findSubRuleByName(portInfo, remark);

  if (existingSubRule) {
    // 子规则已存在，需要更新
    // 构建新的子规则列表（保留其他，更新这一个）
    const newProxyList = portInfo.subRules.map(sub => {
      if (sub.name === remark) {
        // 更新这个子规则
        return {
          Enable: options.enable !== undefined ? options.enable : sub.enabled,
          Key: sub.name, // 使用名称作为 Key
          Remark: remark,
      ...
      return {
        ...
        Key: sub.name,
        Remark: sub.name,
        WebServiceType: sub.type,
        Domains: sub.domains,
        Locations: sub.targets,
        LocationInsecureSkipVerify: true,
        EnableAccessLog: true,
        CorazaWAFInstance: '',
        LogLevel: 4,
    ...
    const updateResult = await updateRule({
      ruleKey: portInfo.key,
      ruleName: portInfo.name,
      listenPort: port,
      network: portInfo.network,
    ...
    // 子规则不存在，添加新规则
    const addResult = await addSubRule(portInfo.key, {
      remark,
      serviceType,
      domains,
      locations,

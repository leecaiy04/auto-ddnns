// Lucky 反向代理模块 API 配置（使用 OpenToken 认证）
import { openTokenFetch, API_BASE_URL } from './lucky-api.mjs';

// ==================== 反向代理 API ====================

export const REVERSE_PROXY_API = {
  // 获取所有反向代理规则
  getRules: `${API_BASE_URL}/api/webservice/rules`,

  // 获取统计信息
  getStats: `${API_BASE_URL}/api/webservice/tipinfo`,

  // 获取模块设置
  getSettings: `${API_BASE_URL}/api/webservice/modulesettings/frontend`,

  // 获取分组列表
  getGroups: `${API_BASE_URL}/api/webservice/groups`,

  // 创建/更新Web服务规则
  setRules: `${API_BASE_URL}/api/webservice/rules`,
};
 ...
 */
export async function getReverseProxyRules() {
 ...
 */
export async function getReverseProxyStats() {
  return await openTokenFetch('/api/webservice/tipinfo');
}
 ...
 * @returns {Promise<Object>}
 */
export async function getReverseProxySettings() {
  return await openTokenFetch('/api/webservice/modulesettings/frontend');
}
 ...
 * @returns {Promise<Object>}
 */
export async function getReverseProxyGroups() {
  return await openTokenFetch('/api/webservice/groups');
}
 ...
 */
export async function getAllReverseProxies() {
  const data = await getReverseProxyRules();
  if (data.ret !== 0) {
    throw new Error('Failed to get rules');
  }
  return extractReverseProxies(data.ruleList);
}
 ...
 */
export async function findTargetForDomain(domain) {
  const data = await getReverseProxyRules();
  if (data.ret !== 0) {
    throw new Error('Failed to get rules');
  }
  const proxy = findProxyByDomain(data.ruleList, domain);
 ...
 * @returns {Promise<{ret: number, msg: string}>}
 */
export async function createPort(port, name, options = {}) {
...
export async function updateRule({
  ...
  const requestBody = {
    ...
    GlobalAllowAllThirdAuthUsers: false,
    ProxyList: proxyList,
    DefaultProxy: createDefaultProxy()
  };
...
export async function addSubRule(ruleKey, {
  ...
  // 先获取当前规则
  const rulesData = await getReverseProxyRules();
  const rule = rulesData.ruleList.find(r => r.RuleKey === ruleKey);
 ...
 * @returns {Promise<{ret: number, msg: string}>}
 */
export async function addReverseProxy(ruleKey, remark, domain, target) {
 ...
 * @returns {Promise<{ret: number, msg: string}>}
 */
export async function addRedirect(ruleKey, remark, fromDomain, toUrl) {
  return addSubRule(ruleKey, {
    remark,
 ...
 * @returns {Promise<{ret: number, msg: string}>}
 */
export async function addFileServer(ruleKey, remark, domain, storagePath) {
  return addSubRule(ruleKey, {
    remark,
 ...
 * @returns {Promise<{ret: number, msg: string}>}
 */
export async function deleteRule(ruleKey) {
  return await openTokenFetch(`/api/webservice/rule/${ruleKey}`, {
    method: 'DELETE'
  });
}
 ...
 * @returns {Promise<{ret: number, msg: string}>}
 */
export async function deletePort(port) {
  const data = await getReverseProxyRules();
  const rule = data.ruleList.find(r => r.ListenPort === port);

  if (!rule) {
    throw new Error(`Port ${port} not found`);
  }

  return deleteRule(rule.RuleKey);
}
...
export async function deleteRuleByName(ruleName) {
  const data = await getReverseProxyRules();
  const rule = data.ruleList.find(r => r.RuleName === ruleName);

...
function normalizeProxyHostPayload(options = {}) {
  return {
    ...
    certificate_id: options.certificate_id || 0,
    block_exploits: Boolean(options.block_exploits || false),
    caching_enabled: Boolean(options.caching_enabled || false),
    allow_websocket_upgrade: options.allow_websocket_upgrade !== false,
    http2_support: Boolean(options.http2_support || false),
    hsts_enabled: Boolean(options.hsts_enabled || false),
    hsts_subdomains: Boolean(options.hsts_subdomains || false),
    ssl_forced: Boolean(options.ssl_forced || false),
    meta: options.meta || { letsencrypt_agree: false, dns_challenge: false },
    advanced_config: options.advanced_config || ''
  };
}

...
export async function createProxyHost({
  ...
  http2_support_enabled = http2_support,
  hsts_enabled = false,
  ...
  return await callApi('/nginx/proxy-hosts', 'POST', {
    domain_names,
    forward_host,
    forward_port,
    access_list_id,
    ...
    http2_support,
    letsencrypt_agree: meta.letsencrypt_agree,
    dns_challenge: meta.dns_challenge,
    advanced_config,
    custom_locations,
    meta,
...
export async function updateProxyHost(id, options) {
  return await callApi(`/nginx/proxy-hosts/${id}`, 'PUT', options);
}

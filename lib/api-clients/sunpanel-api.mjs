...
const API_BASE = getEnv('SUNPANEL_API_BASE', 'http://192.168.3.200:20001/openapi/v1');
const API_TOKEN = getEnv('SUNPANEL_API_TOKEN', 'your-sunpanel-api-token');
 ...
 */
export async function createGroup({ title, onlyName }) {
  return await callApi('/itemGroup/create', { title, onlyName });
}
...
export async function getGroupList() {
  return await callApi('/itemGroup/getList', {});
}
 ...
 */
export async function getGroupInfo({ itemGroupID, onlyName }) {
  return await callApi('/itemGroup/getInfo', { itemGroupID, onlyName });
}
 ...
 */
export async function getItemInfo(onlyName) {
  return await callApi('/item/getInfoByOnlyName', { onlyName });
}
...
export async function updateItem({
  ...
  return await callApi('/item/update', data);
}

...
async function callApi(endpoint, data = {}, config = null) {
  ...
  if (result.code !== 0) {
    throw new Error(`SunPanel API ${endpoint} @ ${resolvedConfig.apiBase} failed: code=${result.code} msg=${result.msg}`);
  }
...
export async function createGroup({ title, onlyName }, config = null) {
  const data = await callApi('/itemGroup/create', { title, onlyName }, config);
  return data ?? {};
}
